...
export default function LogoCarousel({ logos, reverse = false, grayscale = false, speed = 30 }: LogoCarouselProps) {
  ...
  return (
    <div className="relative overflow-hidden w-full">
      ...
      <div
        className={`flex items-center gap-10 ${reverse ? 'animate-carousel-reverse' : 'animate-carousel'}`}
        style={{ animationDuration: `${speed}s` }}
      >
        {duplicated.map((logo, i) => (
          <div
            key={i}
            className={`flex-shrink-0 flex items-center justify-center min-w-[120px] h-16 px-4 group cursor-default ${
...
function LogoItem({ logo }: { logo: Logo }) {
  return (
    <div className="flex items-center justify-center gap-1.5 select-none">
      {logo.abbr ? (
        <div
          className="flex items-center justify-center px-3 py-1.5 rounded-lg border-2 font-black text-lg tracking-tighter transition-colors"
          style={{
            borderColor: logo.color || '#003366',
            color: logo.color || '#003366',
          }}
        >
          {logo.abbr}
        </div>
